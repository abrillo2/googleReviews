...
topReviewsContainer.innerHTML+=`<h3 id="reviewsTitle">Google Reviews</h3>
                                <div id="reviewCards" class="grid-container">
                                </div>
                                <a id="showMore" href="javascript:showMoreReviews()">Show More</a>`;

const showMoreElement = document.getElementById("showMore");
data.length > 20 ? showMoreElement.setAttribute("style","display:block") : showMoreElement.setAttribute("style","display:none");
showReviews();
function showMoreReviews() {
    ...
    showReviews();
    $('html, body').animate({
        scrollTop: $('#figure'+reviewInit).offset().top
    }, 1000);
}
